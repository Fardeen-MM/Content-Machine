...
function upsertClient(email, data) {
  const db = getDb();
  if (!email) return insertClient(data);
  ...
  if (existing) {
    ...
    if (data.firm_name) { sets.push('firm_name = ?'); params.push(data.firm_name); }
    if (data.phone) { sets.push('phone = ?'); params.push(data.phone); }
    if (data.practice_areas) { sets.push('practice_areas = ?'); params.push(JSON.stringify(data.practice_areas)); }
    if (data.source) { sets.push('source = ?'); params.push(data.source); }
