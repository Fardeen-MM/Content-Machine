...
function parseMeetingRow(row) {
  ...
  return {
    ...
    extracted_data: safeJsonParse(row.extracted_data, {}),
    coaching_notes: safeJsonParse(row.coaching_notes),
    raw_response: safeJsonParse(row.raw_response)
  };
}
...
function parseClientRow(row) {
  if (!row) return null;
  let practice_areas = [];
  try { if (row.practice_areas) practice_areas = JSON.parse(row.practice_areas); } catch {}
  return { ...row, practice_areas };
}
...
function parseProposalRow(row) {
  if (!row) return null;
  return { ...row, services: safeJsonParse(row.services, []) };
}
...
function getAtoms({ meeting_id, type, limit = 100 } = {}) {
  ...
  return db.prepare(sql).all(...params).map(row => ({
    ...
    used_in: safeJsonParse(row.used_in, [])
  }));
}
...
function upsertPattern(type, description, meetingId) {
  ...
  if (existing) {
    const ids = safeJsonParse(existing.meeting_ids, []);
...
function getPatterns({ type, limit = 100 } = {}) {
  ...
  return db.prepare(sql).all(...params).map(row => ({
    ...row,
    meeting_ids: safeJsonParse(row.meeting_ids, [])
  }));
}
...
function computeClientHealth(clientId) {
  ...
  const coachingNotes = safeJsonParse(latestMeeting?.coaching_notes);
...
function clearDerivedData() {
  ...
  db.exec(`
    ...
    DELETE FROM briefs;
    DELETE FROM pestering_log;
    DELETE FROM deal_outcomes;
...
function updateDealOutcome(id, data) {
  const d = getDb();
  const existing = d.prepare('SELECT * FROM deal_outcomes WHERE id = ?').get(id);
  if (!existing) return null;
  const fields = [];
  const params = [];
  if (data.outcome !== undefined) { fields.push('outcome = ?'); params.push(data.outcome); }
  if (data.monthly_value !== undefined) { fields.push('monthly_value = ?'); params.push(data.monthly_value); }
  if (data.loss_reason !== undefined) { fields.push('loss_reason = ?'); params.push(data.loss_reason); }
  if (data.what_worked !== undefined) { fields.push('what_worked = ?'); params.push(data.what_worked); }
  if (data.what_failed !== undefined) { fields.push('what_failed = ?'); params.push(data.what_failed); }
  if (data.practice_area !== undefined) { fields.push('practice_area = ?'); params.push(data.practice_area); }
  if (data.source_channel !== undefined) { fields.push('source_channel = ?'); params.push(data.source_channel); }
  if (fields.length === 0) return existing;
  params.push(id);
  d.prepare(`UPDATE deal_outcomes SET ${fields.join(', ')} WHERE id = ?`).run(...params);
  return d.prepare('SELECT d.*, c.name as client_name, c.firm_name FROM deal_outcomes d LEFT JOIN clients c ON d.client_id = c.id WHERE d.id = ?').get(id);
}

function deleteDealOutcome(id) {
  const d = getDb();
  const existing = d.prepare('SELECT * FROM deal_outcomes WHERE id = ?').get(id);
  if (!existing) return null;
  d.prepare('DELETE FROM deal_outcomes WHERE id = ?').run(id);
  return existing;
}
...
module.exports = {
  ...
  insertDealOutcome, getDealOutcomes, updateDealOutcome, deleteDealOutcome,
