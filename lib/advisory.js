...
async function extractDiscoveryFramework() {
  ...
  const callSummaries = bestCalls.map(m => {
    ...
    const toList = v => Array.isArray(v) ? v : (v ? [String(v)] : []);
    return `Score: ${cn.score}/100 | ${m.client_name || m.title}
    Strengths: ${toList(cn.strengths).join('; ')}
    Pain points uncovered: ${toList(ed.pain_points).join('; ')}
    Services discussed: ${toList(ed.services_discussed).join(', ')}
    Objections: ${toList(ed.objections).join('; ')}
