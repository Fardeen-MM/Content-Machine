...
function createPesterSchedule(clientId, stage, startDate) {
  ...
  for (const item of stageConfig.schedule) {
    ...
    try {
      ...
      console.error(`[pester] Failed to insert entry for client #${clientId}:`, err.message);
    }
  }
...
async function generatePesterMessage(entryId) {
  ...
  const meetingContext = meetings.map(m => {
    ...
    return `${m.date?.slice(0, 10)}: ${m.summary || m.title}${Array.isArray(ed.pain_points) && ed.pain_points.length ? ' | Pain: ' + ed.pain_points.join(', ') : ''}`;
  }).join('\n');
  ...
  try {
    ...
    let practiceAreas = entry.practice_areas || '';
    if (typeof practiceAreas === 'string' && practiceAreas.startsWith('[')) {
      try { practiceAreas = JSON.parse(practiceAreas).join(', '); } catch {}
    }
    if (Array.isArray(practiceAreas)) practiceAreas = practiceAreas.join(', ');
    const clientKeywords = String(practiceAreas).toLowerCase().split(/[,;/]+/).map(s => s.trim()).filter(Boolean);
