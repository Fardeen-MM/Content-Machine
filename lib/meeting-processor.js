...
async function processMeeting(meetingRow) {
  ...
  // Step 1: Classify
  let classification = { type: 'unknown', confidence: 0 };
  try {
    classification = await classifyMeeting(meetingRow.title, transcript);
    console.log(`[processor] Classified as: ${classification.type} (${classification.confidence}%)`);
  } catch (err) {
    console.error(`[processor] Classification failed for #${meetingRow.id}:`, err.message);
  }
  ...
  let extracted = {};
  try {
    extracted = await extractMeetingData(transcript, meetingRow.title);
    console.log(`[processor] Extracted ${extracted.action_items?.length || 0} action items`);
  } catch (err) {
    console.error(`[processor] Extraction failed for #${meetingRow.id}:`, err.message);
  }
  ...
  if (classification.type !== 'internal' && classification.type !== 'unknown') {
    try {
      const rawAtoms = await extractContentAtoms(transcript, extracted);
      atoms = Array.isArray(rawAtoms) ? rawAtoms : [];
      console.log(`[processor] Found ${atoms.length} content atoms`);
    } catch (err) {
      console.error(`[processor] Content atom extraction failed for #${meetingRow.id}:`, err.message);
    }
  }
  ...
  try {
    ...
    const rawPatterns = await analyzePatterns(extracted, existingPatterns);
    newPatterns = Array.isArray(rawPatterns) ? rawPatterns : [];
    console.log(`[processor] Identified ${newPatterns.length} patterns`);
  } catch (err) {
    console.error(`[processor] Pattern analysis failed for #${meetingRow.id}:`, err.message);
  }
  ...
  if (['discovery', 'review'].includes(classification.type)) {
    try {
      ...
      console.error(`[processor] Coaching generation failed for #${meetingRow.id}:`, err.message);
    }
  }
  ...
  for (const atom of (atoms || [])) {
    if (!atom.type || !atom.content) continue;
    db.insertAtom({
      meeting_id: meetingRow.id,
      type: atom.type,
