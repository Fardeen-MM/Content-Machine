...
Return JSON only (no markdown fences):
{
  ...
  "executive_summary": "2-3 paragraph executive summary addressing their specific pain points and how we solve them. Include ROI projections naturally.",
  "recommended_services": [
    {
      ...
      "monthly_cost": estimated monthly cost as number (use Mortar Metrics pricing: SEO $2000-5000, PPC $1500-4000 + ad spend, Web Design $3000-8000 one-time, Lead Gen $2000-5000, GHL/Automation included, Voice Agent included. Typical deal $3K-$5K/mo, goal $10K/mo)
    }
  ],
  "roi_section": "A paragraph framing the investment vs expected return using the ROI projections provided. Show 3 scenarios. Make the numbers feel tangible.",
  "guarantee": "A results-based guarantee statement (e.g. 'If we don't generate X qualified leads in 90 days, the next month is on us'). Tie to specific deliverables, not vague promises.",
...
- ROI section must use the projections provided, not made-up numbers
- Guarantee must be specific and deliverable-based, never promise revenue directly`;

  const kb = getKnowledgeBase();
  const text = await callClaude({
    model: SONNET,
    system: `You are the proposal writer for Mortar Metrics. Generate targeted proposals based on discovery call data. Return JSON only.\n\nBUSINESS CONTEXT:\n${kb}`,
    prompt,
    maxTokens: 3000
  });
  ...
  markdown += `## Projected ROI\n\n${parsed.roi_section || ''}\n\n`;
  markdown += `| Scenario | Cases/Month | Monthly Revenue |\n|----------|------------|----------------|\n`;
  markdown += `| Conservative | ${roi.scenarios.worst.cases} | $${roi.scenarios.worst.revenue.toLocaleString()} |\n`;
  markdown += `| Expected | ${roi.scenarios.expected.cases} | $${roi.scenarios.expected.revenue.toLocaleString()} |\n`;
  markdown += `| Strong | ${roi.scenarios.strong.cases} | $${roi.scenarios.strong.revenue.toLocaleString()} |\n\n`;
